...
於 `src/url_to_parquet/__init__.py` 中暴露主要功能，此乃用户所見之第一面。
...
def main():
    ...
    parser.add_argument("-i", "--input-file", type=Path, help="包含URL的文本文件")
...
一個強大的工具，可將任意 URL 轉換為結構化的 Parquet、CSV、JSON 或 Markdown 文件。
...
| 封裝層次 / 模組 | FetchService | ValidationService | PersistenceService |
| ---------------- | ------------ | ----------------- | ----------------- |
| 核心庫 (core.py) | 抓取/調度外部資源或後續擴展的抓取器介面 | 參數 & URL 合規檢查（可擴展內容品質驗證） | 任務產出路徑、結果文件寫入 |
| CLI (cli.py) | 觸發批次抓取流程 | 基礎輸入校驗 | 作業執行結果落地（標準輸出 + 文件） |
| API (api.py) | 後台任務啟動/瀏覽器渲染（可選） | 任務狀態/結果有效性與錯誤分類 | 任務暫存 / 結果檔案 / 日誌持久化（可升級 Redis/DB） |
| Docker 化 | 提供一致執行環境確保抓取與驗證一致性 | — | 昇華部署交付（影像版本即資產） |
| CI/CD | 觸發測試與依賴更新檢查 | 靜態分析、型別與品質檢查 | 發布制品（PyPI / 映像）與追蹤版本 |

> 此映射協助將封裝導向與整體系統三層服務抽象對齊，未來擴展時可直接抽離 fetch/validation/persistence 為獨立套件或微服務。
