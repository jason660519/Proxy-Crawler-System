...
| 流程階段 / 功能 | FetchService | ValidationService | PersistenceService |
| ---------------- | ------------ | ----------------- | ----------------- |
| URL 抓取 (下載 HTML) | 發送 HTTP/瀏覽器抓取、節流控制 | （基本可用性/內容完整性檢查） | 原始內容來源記錄、抓取快取 |
| 正文抽取 / Markdown 轉換 | 多引擎抽取、失敗降級策略 | 內容品質/長度/語言判定 | 抽取結果與元資料存儲、去重索引 |
| 結構化輸出 (JSON/CSV/Parquet) | （可能需外部 schema 參考） | schema 合規/欄位驗證 | 最終文件落地、版本與 checksum |
| 進度 / 日誌推送 | 取得實時任務狀態 | 狀態異常/重試策略評估 | 狀態持久化、事件時間序列 |
| 去重與冪等 | 快取命中與重用 | 校驗 checksum 與一致性 | 存儲哈希索引、任務結果引用 |

> 後端可按三層抽象劃分模組：fetch_pipeline / validation_rules / artifact_repository。
